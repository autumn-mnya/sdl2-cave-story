...
{
	sdlstartdirectdrawElementHandlers.push_back(handler);
}

void ExecuteSDLStartDirectDrawElementHandlers()
{
	for (const auto& handler : sdlstartdirectdrawElementHandlers)
	{
		handler();
	}
}
...
bool RenderBackend::initWindow(int width, int height, int magnification)
{
	...
	ExecuteSDLStartDirectDrawElementHandlers();
